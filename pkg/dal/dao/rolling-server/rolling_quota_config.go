...
var _ RollingQuotaConfigInterface = new(RollingQuotaConfigDao)

// RollingQuotaConfigDao rolling quota config RollingQuotaConfigDao.
type RollingQuotaConfigDao struct {
	Orm   orm.Interface
	IDGen idgenerator.IDGenInterface
}

// CreateWithTx create rolling quota config with tx.
func (d RollingQuotaConfigDao) CreateWithTx(kt *kit.Kit, tx *sqlx.Tx, models []tablers.RollingQuotaConfigTable) (
	[]string, error) {
	...
	sql := fmt.Sprintf(`INSERT INTO %s (%s)	VALUES(%s)`, models[0].TableName(),
		tablers.RollingQuotaConfigColumns.ColumnExpr(), tablers.RollingQuotaConfigColumns.ColonNameExpr())
...
func (d RollingQuotaConfigDao) List(kt *kit.Kit, opt *types.ListOption) (*rsproto.RollingQuotaConfigListResult, error) {
	...
	if err := opt.Validate(filter.NewExprOption(filter.RuleFields(tablers.RollingQuotaConfigColumns.ColumnTypes())),
	...
	if opt.Page.Count {
		...
		sql := fmt.Sprintf(`SELECT COUNT(*) FROM %s %s`, table.RollingQuotaConfigTable, whereExpr)

		count, err := d.Orm.Do().Count(kt.Ctx, sql, whereValue)
		if err != nil {
			logs.ErrorJson("count rolling quota config failed, err: %v, filter: %v, rid: %s", err, opt.Filter, kt.Rid)
			return nil, err
		}
	...
	sql := fmt.Sprintf(`SELECT %s FROM %s %s %s`, tablers.RollingQuotaConfigColumns.FieldsNamedExpr(opt.Fields),
		table.RollingQuotaConfigTable, whereExpr, pageExpr)

	details := make([]tablers.RollingQuotaConfigTable, 0)
...
func (d RollingQuotaConfigDao) DeleteWithTx(kt *kit.Kit, tx *sqlx.Tx, expr *filter.Expression) error {
	if expr == nil {
	...
	sql := fmt.Sprintf(`DELETE FROM %s %s`, table.RollingQuotaConfigTable, whereExpr)

	if _, err = d.Orm.Txn(tx).Delete(kt.Ctx, sql, whereValue); err != nil {
		logs.ErrorJson("delete rolling quota config failed, err: %v, filter: %v, rid: %s", err, expr, kt.Rid)
		return err
	}
