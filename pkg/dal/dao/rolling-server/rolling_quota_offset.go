...
type RollingQuotaOffsetInterface interface {
	...
	UpdateWithTx(kt *kit.Kit, tx *sqlx.Tx, expr *filter.Expression, model *tablers.RollingQuotaOffsetTable) error
	List(kt *kit.Kit, opt *types.ListOption) (*rsproto.RollingQuotaOffsetListResult, error)
...
func (d RollingQuotaOffsetDao) UpdateWithTx(kt *kit.Kit, tx *sqlx.Tx, filterExpr *filter.Expression,
...
func (d RollingQuotaOffsetDao) List(kt *kit.Kit, opt *types.ListOption) (*rsproto.RollingQuotaOffsetListResult, error) {
	if opt == nil {
	...
	if opt.Page.Count {
		...
		return &rsproto.RollingQuotaOffsetListResult{Count: count}, nil
	}
	...
	return &rsproto.RollingQuotaOffsetListResult{Count: 0, Details: details}, nil
}
