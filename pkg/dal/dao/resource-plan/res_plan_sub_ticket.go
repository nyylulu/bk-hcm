...
var _ ResPlanSubTicketInterface = new(ResPlanSubTicketDao)

// ResPlanSubTicketDao resource plan sub_ticket ResPlanSubTicketDao.
type ResPlanSubTicketDao struct {
	Orm   orm.Interface
	IDGen idgenerator.IDGenInterface
	Audit audit.Interface
}

// CreateWithTx create resource plan sub_ticket with tx.
func (d ResPlanSubTicketDao) CreateWithTx(kt *kit.Kit, tx *sqlx.Tx, models []rpst.ResPlanSubTicketTable) ([]string,
	error) {
	if len(models) == 0 {
	...
	sql := fmt.Sprintf(`INSERT INTO %s (%s)	VALUES(%s)`, models[0].TableName(),
		rpst.ResPlanSubTicketColumns.ColumnExpr(), rpst.ResPlanSubTicketColumns.ColonNameExpr())
...
func (d ResPlanSubTicketDao) List(kt *kit.Kit, opt *types.ListOption) (*rtypes.RPSubTicketListResult, error) {
	if opt == nil {
		return nil, errf.New(errf.InvalidParameter, "list res plan sub ticket options is nil")
	}

	exprOpt := filter.NewExprOption(
		filter.RuleFields(rpst.ResPlanSubTicketColumns.ColumnTypes()),
	...
	if opt.Page.Count {
		...
		sql := fmt.Sprintf(`SELECT COUNT(*) FROM %s %s`, table.ResPlanSubTicketTable, whereExpr)

		count, err := d.Orm.Do().Count(kt.Ctx, sql, whereValue)
		if err != nil {
			logs.ErrorJson("count res plan sub ticket failed, err: %v, filter: %v, rid: %s", err, opt.Filter, kt.Rid)
			return nil, err
		}

		return &rtypes.RPSubTicketListResult{Count: count}, nil
	}
	...
	sql := fmt.Sprintf(`SELECT %s FROM %s %s %s`, rpst.ResPlanSubTicketColumns.FieldsNamedExpr(opt.Fields),
		table.ResPlanSubTicketTable, whereExpr, pageExpr)

	details := make([]rpst.ResPlanSubTicketTable, 0)
	if err = d.Orm.Do().Select(kt.Ctx, &details, sql, whereValue); err != nil {
		return nil, err
	}

	return &rtypes.RPSubTicketListResult{Count: 0, Details: details}, nil
}

// DeleteWithTx delete resource plan sub_ticket with tx.
func (d ResPlanSubTicketDao) DeleteWithTx(kt *kit.Kit, tx *sqlx.Tx, expr *filter.Expression) error {
	if expr == nil {
	...
	sql := fmt.Sprintf(`DELETE FROM %s %s`, table.ResPlanSubTicketTable, whereExpr)

	if _, err = d.Orm.Txn(tx).Delete(kt.Ctx, sql, whereValue); err != nil {
		logs.ErrorJson("delete resource plan sub ticket failed, err: %v, filter: %v, rid: %s", err, expr, kt.Rid)
		return err
	}
