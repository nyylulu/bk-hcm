...
// LbTCloudZiyanUrlRuleDao lb tcloud url rule dao.
type LbTCloudZiyanUrlRuleDao struct {
	Orm   orm.Interface
...
func (dao LbTCloudZiyanUrlRuleDao) BatchCreateWithTx(kt *kit.Kit, tx *sqlx.Tx, models []*tablelb.TCloudZiyanLbUrlRuleTable) (
	...
	sql := fmt.Sprintf(`INSERT INTO %s (%s)	VALUES(%s)`, tableName,
		tablelb.TCloudZiyanLbUrlRuleColumns.ColumnExpr(), tablelb.TCloudZiyanLbUrlRuleColumns.ColonNameExpr())

	if err = dao.Orm.Txn(tx).BulkInsert(kt.Ctx, sql, models); err != nil {
		logs.Errorf("[tcloud-ziyan] insert %s failed, err: %v, rid: %s", tableName, err, kt.Rid)
	...
	if err != nil {
		logs.Errorf("[tcloud-ziyan] fail to list lb for audit, err: %v, rid: %s", err, kt.Rid)
		return nil, err
	}
	...
	if err = dao.Audit.BatchCreateWithTx(kt, tx, audits); err != nil {
		logs.Errorf("[tcloud-ziyan] batch create audit failed, err: %v, rid: %s", err, kt.Rid)
		return nil, err
	}
	return ids, nil
}
...
func (dao LbTCloudZiyanUrlRuleDao) UpdateByIDWithTx(kt *kit.Kit, tx *sqlx.Tx, id string,
...
func (dao LbTCloudZiyanUrlRuleDao) List(kt *kit.Kit, opt *types.ListOption) (*types.ListResult[*tablelb.TCloudZiyanLbUrlRuleTable], error) {
	...
	if opt.Page.Count {
		...
		sql := fmt.Sprintf(`SELECT COUNT(*) FROM %s %s`, table.TCloudZiyanLbUrlRuleTable, whereExpr)
		...
		return &types.ListResult[*tablelb.TCloudZiyanLbUrlRuleTable]{Count: count}, nil
	}
	...
	sql := fmt.Sprintf(`SELECT %s FROM %s %s %s`, tablelb.TCloudZiyanLbUrlRuleColumns.FieldsNamedExpr(opt.Fields),
		table.TCloudZiyanLbUrlRuleTable, whereExpr, pageExpr)

	details := make([]*tablelb.TCloudZiyanLbUrlRuleTable, 0)
...
func (dao LbTCloudZiyanUrlRuleDao) DeleteWithTx(kt *kit.Kit, tx *sqlx.Tx, expr *filter.Expression) error {
	if expr == nil {
	...
	sql := fmt.Sprintf(`DELETE FROM %s %s`, table.TCloudZiyanLbUrlRuleTable, whereExpr)
