/*
 * Tencent is pleased to support the open source community by making 蓝鲸 available.
 * Copyright (C) 2017-2018 THL A29 Limited, a Tencent company. All rights reserved.
 * Licensed under the MIT License (the "License"); you may not use this file except
 * in compliance with the License. You may obtain a copy of the License at
 * http://opensource.org/licenses/MIT
 * Unless required by applicable law or agreed to in writing, software distributed under
 * the License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND,
 * either express or implied. See the License for the specific language governing permissions and
 * limitations under the License.
 */

// Package util common util
package util

import (
	"encoding/json"
	"regexp"
	"strings"
	"unicode/utf8"

	"hcm/pkg"
	"hcm/pkg/criteria/errors"
	"hcm/pkg/logs"
)

// ValidPropertyOption valid property field option
func ValidPropertyOption(propertyType string, option interface{}, errProxy errors.DefaultCCErrorIf) error {
	switch propertyType {
	case pkg.FieldTypeEnum:
		return ValidFieldTypeEnumOption(option, errProxy)
	case pkg.FieldTypeInt:
		return ValidFieldTypeIntOption(option, errProxy)
	case pkg.FieldTypeList:
		return ValidFieldTypeListOption(option, errProxy)
	case pkg.FieldTypeLongChar, pkg.FieldTypeSingleChar:
		return ValidFieldRegularExpressionOption(option, errProxy)
	}
	return nil
}

// ValidFieldTypeEnumOption valid enum option
func ValidFieldTypeEnumOption(option interface{}, errProxy errors.DefaultCCErrorIf) error {
	if nil == option {
		return errProxy.Errorf(pkg.CCErrCommParamsLostField, "option")
	}

	arrOption, ok := option.([]interface{})
	if false == ok {
		logs.Errorf(" option %v not enum option", option)
		return errProxy.Errorf(pkg.CCErrCommParamsIsInvalid, "option")
	}

	if len(arrOption) > pkg.AttributeOptionArrayMaxLength {
		logs.Errorf(" option array length %d exceeds max length %d", len(arrOption),
			pkg.AttributeOptionArrayMaxLength)
		return errProxy.Errorf(pkg.CCErrCommValExceedMaxFailed, "option", pkg.AttributeOptionArrayMaxLength)
	}
	for _, o := range arrOption {
		mapOption, ok := o.(map[string]interface{})
		if false == ok || mapOption == nil {
			logs.Errorf(" option %v not enum option, enum option item must id and name", option)
			return errProxy.Errorf(pkg.CCErrCommParamsIsInvalid, "option")
		}
		idVal, idOk := mapOption["id"]
		if !idOk || idVal == "" {
			logs.Errorf("enum option id can't be empty", option)
			return errProxy.Errorf(pkg.CCErrCommParamsNeedSet, "option id")
		}
		if idValStr, ok := idVal.(string); !ok {
			logs.Errorf("idVal %v not string", idVal)
			return errProxy.Errorf(pkg.CCErrCommParamsNeedString, "option id")
		} else if pkg.AttributeOptionValueMaxLength < utf8.RuneCountInString(idValStr) {
			logs.Errorf(" option id %s length %d exceeds max length %d", idValStr, utf8.RuneCountInString(idValStr),
				pkg.AttributeOptionValueMaxLength)
			return errProxy.Errorf(pkg.CCErrCommValExceedMaxFailed, "option id",
				pkg.AttributeOptionValueMaxLength)
		}
		nameVal, nameOk := mapOption["name"]
		if !nameOk || nameVal == "" {
			logs.Errorf("enum option name can't be empty", option)
			return errProxy.Errorf(pkg.CCErrCommParamsNeedSet, "option name")
		}
		switch mapOption["type"] {
		case "text":
			if nameValStr, ok := nameVal.(string); !ok {
				logs.Errorf(" nameVal %v not string", nameVal)
				return errProxy.Errorf(pkg.CCErrCommParamsNeedString, "option name")
			} else if pkg.AttributeOptionValueMaxLength < utf8.RuneCountInString(nameValStr) {
				logs.Errorf(" option name %s length %d exceeds max length %d", nameValStr,
					utf8.RuneCountInString(nameValStr), pkg.AttributeOptionValueMaxLength)
				return errProxy.Errorf(pkg.CCErrCommValExceedMaxFailed, "option name",
					pkg.AttributeOptionValueMaxLength)
			}
		default:
			logs.Errorf("enum option type must be 'text', current: %v", mapOption["type"])
			return errProxy.Errorf(pkg.CCErrCommParamsIsInvalid, "option type")
		}
	}

	return nil
}

// ValidFieldTypeIntOption 验证枚举选项
func ValidFieldTypeIntOption(option interface{}, errProxy errors.DefaultCCErrorIf) error {
	if nil == option {
		return errProxy.Errorf(pkg.CCErrCommParamsLostField, "option")
	}

	tmp, ok := option.(map[string]interface{})
	if false == ok {
		return errProxy.Errorf(pkg.CCErrCommParamsIsInvalid, "option")
	}

	{
		// min
		min, ok := tmp["min"]
		maxVal := 99999999999 // default
		minVal := -9999999999 // default
		err := errProxy.Error(pkg.CCErrCommParamsNeedInt)

		isPass := false
		if ok {
			switch d := min.(type) {
			case string:
				if 0 == len(d) {
					isPass = true
				}
				if 11 < len(d) {
					return errProxy.Errorf(pkg.CCErrCommOverLimit, "option.min")
				}
			}

			if !isPass {
				if ok := IsNumeric(min); !ok {
					return errProxy.Errorf(pkg.CCErrCommParamsNeedInt, "option.min")
				}
				minVal, err = GetIntByInterface(min)
				if nil != err {
					return errProxy.Errorf(pkg.CCErrCommParamsNeedInt, "option.min")
				}
			}
		}

		// max
		max, ok := tmp["max"]
		if ok {
			isPass := false
			switch d := max.(type) {
			case string:
				if 0 == len(d) {
					isPass = true
				}
				if 11 < len(d) {
					return errProxy.Errorf(pkg.CCErrCommOverLimit, "option.max")
				}
			}
			if !isPass {
				if ok := IsNumeric(max); !ok {
					return errProxy.Errorf(pkg.CCErrCommParamsNeedInt, "option.max")
				}
				maxVal, err = GetIntByInterface(max)
				if nil != err {
					return errProxy.Errorf(pkg.CCErrCommParamsNeedInt, "option.max")
				}
			}
		}

		if minVal > maxVal {
			return errProxy.Errorf(pkg.CCErrCommParamsIsInvalid, "option.max")
		}
	}

	return nil
}

// ValidFieldTypeListOption 验证枚举值
func ValidFieldTypeListOption(option interface{}, errProxy errors.DefaultCCErrorIf) error {
	if nil == option {
		return errProxy.Errorf(pkg.CCErrCommParamsLostField, "option")
	}

	arrOption, ok := option.([]interface{})
	if false == ok {
		logs.Errorf(" option %v not string type list option", option)
		return errProxy.Errorf(pkg.CCErrCommParamsIsInvalid, "option")
	}
	if len(arrOption) > pkg.AttributeOptionArrayMaxLength {
		logs.Errorf(" option array length %d exceeds max length %d", len(arrOption),
			pkg.AttributeOptionArrayMaxLength)
		return errProxy.Errorf(pkg.CCErrCommValExceedMaxFailed, "option", pkg.AttributeOptionArrayMaxLength)
	}

	for _, val := range arrOption {
		switch value := val.(type) {
		case string: // 只可以是字符类型
			if pkg.AttributeOptionValueMaxLength < utf8.RuneCountInString(value) {
				logs.Errorf(" option value %s length %d exceeds max length %d", value, utf8.RuneCountInString(value),
					pkg.AttributeOptionValueMaxLength)
				return errProxy.Errorf(pkg.CCErrCommValExceedMaxFailed, "option",
					pkg.AttributeOptionValueMaxLength)
			}
		default:
			logs.Errorf(" option %v not string type list option", option)
			return errProxy.Errorf(pkg.CCErrCommParamsIsInvalid, "list option need string type item")
		}
	}

	return nil
}

// ValidFieldRegularExpressionOption 验证正则表达式
func ValidFieldRegularExpressionOption(option interface{}, errProxy errors.DefaultCCErrorIf) error {
	// check regular is legal
	if option == nil {
		return nil
	}

	regular, ok := option.(string)
	if !ok {
		logs.Errorf("variable type conversion error")
		return errProxy.Errorf(pkg.CCIllegalRegularExpression, "option")
	}

	if len(regular) == 0 {
		return nil
	}

	_, err := regexp.Compile(regular)
	if err != nil {
		logs.Errorf("regular expression is wrong, regular expression is:%s, err:%s", regular, err)
		return errProxy.Errorf(pkg.CCErrorCheckRegularFailed)
	}

	return nil
}

// IsStrProperty  is string property
func IsStrProperty(propertyType string) bool {
	if pkg.FieldTypeLongChar == propertyType || pkg.FieldTypeSingleChar == propertyType {
		return true
	}

	return false
}

// IsInnerObject is inner object model
func IsInnerObject(objID string) bool {
	switch objID {
	case pkg.BKInnerObjIDApp:
		return true
	case pkg.BKInnerObjIDHost:
		return true
	case pkg.BKInnerObjIDModule:
		return true
	case pkg.BKInnerObjIDPlat:
		return true
	case pkg.BKInnerObjIDProc:
		return true
	case pkg.BKInnerObjIDSet:
		return true
	}

	return false
}

// IsNumeric judges if value is a number
func IsNumeric(val interface{}) bool {
	switch val.(type) {
	case int, int8, int16, int32, int64, uint, uint8, uint16, uint32, uint64, float32, float64, json.Number:
		return true
	}

	return false
}

var mainlineNameRegexp = regexp.MustCompile(pkg.FieldTypeMainlineRegexp)

// ValidTopoNameField validate business topology name, including set and service templates that may generate them
func ValidTopoNameField(name string, nameField string, errProxy errors.DefaultCCErrorIf) (string, error) {
	name = strings.Trim(name, " ")

	if len(name) == 0 {
		return name, errProxy.CCErrorf(pkg.CCErrCommParamsNeedSet, nameField)
	}

	if utf8.RuneCountInString(name) > pkg.MainlineNameFieldMaxLength {
		return name, errProxy.CCErrorf(pkg.CCErrCommValExceedMaxFailed, nameField, pkg.MainlineNameFieldMaxLength)
	}

	match := mainlineNameRegexp.MatchString(name)
	if !match {
		return name, errProxy.CCErrorf(pkg.CCErrCommParamsInvalid, nameField)
	}

	return name, nil
}

// ValidMustSetStringField valid if the value is of string type and is not empty
func ValidMustSetStringField(value interface{}, field string, errProxy errors.DefaultCCErrorIf) (string, error) {
	switch val := value.(type) {
	case string:
		if len(val) == 0 {
			return val, errProxy.Errorf(pkg.CCErrCommParamsNeedSet, field)
		}
		return val, nil
	default:
		return "", errProxy.New(pkg.CCErrCommParamsNeedString, field)
	}
}

// ValidModelIDField validate model related id field, like classification id, attribute id, group id...
func ValidModelIDField(value interface{}, field string, errProxy errors.DefaultCCErrorIf) error {
	strValue, err := ValidMustSetStringField(value, field, errProxy)
	if err != nil {
		return err
	}

	if utf8.RuneCountInString(strValue) > pkg.AttributeIDMaxLength {
		return errProxy.Errorf(pkg.CCErrCommValExceedMaxFailed, field, pkg.AttributeIDMaxLength)
	}

	match, err := regexp.MatchString(pkg.FieldTypeStrictCharRegexp, strValue)
	if nil != err {
		return err
	}
	if !match {
		return errProxy.Errorf(pkg.CCErrCommParamsIsInvalid, field)
	}
	return nil
}

// ValidModelNameField validate model related name field, like classification name, attribute name, group name...
func ValidModelNameField(value interface{}, field string, errProxy errors.DefaultCCErrorIf) error {
	strValue, err := ValidMustSetStringField(value, field, errProxy)
	if err != nil {
		return err
	}

	if utf8.RuneCountInString(strValue) > pkg.AttributeNameMaxLength {
		return errProxy.Errorf(pkg.CCErrCommValExceedMaxFailed, field, pkg.AttributeNameMaxLength)
	}
	return nil
}
