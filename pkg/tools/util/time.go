...
var (
	// 需要转换的时间的标志
	convTimeFields = []string{pkg.CreateTimeField, pkg.LastTimeField, pkg.ConfirmTimeField}
)
...
func TimeStrToTimePtr(str string) (*time.Time, error) {
	parseTime, err := time.ParseInLocation(pkg.TimeDayTransferModel, str, time.Local)
	if err != nil {
		return nil, err
	}
...
func FormatPeriod(period string) (string, error) {
	if pkg.Infinite == period || "" == period {
		return pkg.Infinite, nil
	}
