...
func TestGetActionLanguage(t *testing.T) {
	...
	req.Header.Set(pkg.BKHTTPLanguage, "cn")
	language = GetLanguage(restful.NewRequest(req).Request.Header)
	require.Equal(t, "cn", language)

	req.Header.Set(pkg.BKHTTPLanguage, "cnn")
	language = GetLanguage(restful.NewRequest(req).Request.Header)
...
func TestHeader(t *testing.T) {
	header := http.Header{}
	header.Set(pkg.BKHTTPLanguage, "zh")
	header.Set(pkg.BKHTTPHeaderUser, "user")
	header.Set(pkg.BKHTTPOwnerID, "owner")
	header.Set(pkg.BKHTTPCCRequestID, "rid")
