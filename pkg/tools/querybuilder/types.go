...
func (c Condition) ToMgo() (mgoOperator string, err error) {
	switch c {
	case ConditionOr:
		return pkg.BKDBOR, nil
	case ConditionAnd:
		return pkg.BKDBAND, nil
	default:
...
func (r AtomRule) ToMgo() (mgoFiler map[string]interface{}, key string, err error) {
	...
	case OperatorDatetimeLess:
		ts, err := time.Parse(timeLayout, r.Value.(string))
		if err != nil {
	...
	case OperatorBeginsWith:
		filter[r.Field] = map[string]interface{}{
			pkg.BKDBLIKE: fmt.Sprintf("^%s", r.Value),
		}
	case OperatorNotBeginsWith:
		filter[r.Field] = map[string]interface{}{
			pkg.BKDBNot: map[string]interface{}{pkg.BKDBLIKE: fmt.Sprintf("^%s", r.Value)},
		}
	...
	case OperatorIsEmpty:
		// array empty
		filter[r.Field] = map[string]interface{}{
			pkg.BKDBEQ: make([]interface{}, 0),
		}
	case OperatorIsNotEmpty:
		// array not empty
		filter[r.Field] = map[string]interface{}{
			pkg.BKDBNE: make([]interface{}, 0),
		}
	case OperatorIsNull:
		filter[r.Field] = map[string]interface{}{
			pkg.BKDBEQ: nil,
