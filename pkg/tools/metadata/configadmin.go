...
func (option *BuiltInModuleDeleteOption) Validate() error {
	...
	if option.ModuleKey == pkg.SystemIdleModuleKey || option.ModuleKey == pkg.SystemFaultModuleKey ||
		option.ModuleKey == pkg.SystemRecycleModuleKey {
...
func (b *BaseCfgItem) ValidateValueFormat() error {
	...
	if b.IsExceedMaxLength() {
		return fmt.Errorf("value length can't exceed %s", pkg.AttributeOptionMaxLength)
	}
	return nil
}
...
func (b *BaseCfgItem) IsExceedMaxLength() bool {
	if len(strings.TrimSpace(b.Value)) > pkg.AttributeOptionMaxLength {
		return true
	}
	return false
}
