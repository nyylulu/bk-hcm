...
func NewSuccessResp(data interface{}) *Response {
	return &Response{
		BaseResp: BaseResp{Result: true, Code: pkg.CCSuccess, ErrMsg: pkg.CCSuccessStr},
		Data:     data,
	}
}
...
func (tc *TimeCondition) MergeTimeCondition(condition map[string]interface{}) (map[string]interface{}, error) {
	...
	if tc.Operator != "and" {
		return nil, errors.New(pkg.CCErrCommParamsInvalid, "time condition oper is invalid")
	}

	if len(tc.Rules) == 0 {
		return nil, errors.New(pkg.CCErrCommParamsNeedSet, "time condition rules not set")
	}

	timeCondition := make(map[string]interface{})
	for _, cond := range tc.Rules {
		if len(cond.Field) == 0 {
			return nil, errors.New(pkg.CCErrCommParamsNeedSet, "time condition field not set")
		}

		if cond.Start == nil && cond.End == nil {
			return nil, errors.New(pkg.CCErrCommParamsInvalid, "time condition start and end both not set")
		}

		if cond.Start == nil {
			timeCondition[cond.Field] = map[string]interface{}{pkg.BKDBLTE: cond.End}
			continue
		}

		if cond.End == nil {
			timeCondition[cond.Field] = map[string]interface{}{pkg.BKDBGTE: cond.Start}
			continue
		}

		if *cond.Start == *cond.End {
			timeCondition[cond.Field] = map[string]interface{}{pkg.BKDBEQ: cond.Start}
			continue
		}

		timeCondition[cond.Field] = map[string]interface{}{pkg.BKDBGTE: cond.Start, pkg.BKDBLTE: cond.End}
	}
	...
	return map[string]interface{}{pkg.BKDBAND: []map[string]interface{}{condition, timeCondition}}, nil
}
...
func (o *QueryInput) convTimeItem(item interface{}) (interface{}, error) {
	...
	case map[string]interface{}:
		...
		if true == ok {
			_, timeTypeOk := arrItem[pkg.BKTimeTypeParseFlag]
			if timeTypeOk {
				delete(arrItem, pkg.BKTimeTypeParseFlag)
			}
...
func (s *SearchInstBatchOption) Validate() (rawError errors.RawErrorInfo) {
	if len(s.IDs) == 0 || len(s.IDs) > pkg.BKMaxInstanceLimit {
		return errors.RawErrorInfo{
			ErrCode: pkg.CCErrArrayLengthWrong,
			Args:    []interface{}{"bk_ids", pkg.BKMaxInstanceLimit},
		}
	}
...
func (f *CommonSearchFilter) Validate() (string, error) {
	...
	if err := f.Page.ValidateLimit(pkg.BKMaxInstanceLimit); err != nil {
