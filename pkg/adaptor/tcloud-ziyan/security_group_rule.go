...
func (t *ZiyanAdpt) CreateSecurityGroupRule(kt *kit.Kit, opt *securitygrouprule.TCloudCreateOption) error {
	...
	if err != nil {
		// 触发审批流的情况
		if berr := errf.GetBPassApprovalErrorf(err); berr != nil {
			logs.Infof("trigger bpass approval, application ID: %v, rid: %s", berr.Message, kt.Rid)
			return berr
		}
		logs.Errorf("create tcloud ziyan security group rule failed, err: %v, rid: %s", err, kt.Rid)
		return err
	}
...
func (t *ZiyanAdpt) DeleteSecurityGroupRule(kt *kit.Kit, opt *securitygrouprule.TCloudDeleteOption) error {
	if opt == nil {
	...
	if err != nil {
		...
		logs.Errorf("delete tcloud ziyan security group rule failed, err: %v, rid: %s", err, kt.Rid)
		return err
	}
...
func (t *ZiyanAdpt) UpdateSecurityGroupRule(kt *kit.Kit, opt *securitygrouprule.TCloudUpdateOption) error {
	if opt == nil {
	...
	if err != nil {
		...
		logs.Errorf("replace tcloud ziyan security group rule failed, err: %v, rid: %s", err, kt.Rid)
		return err
	}
...
func (t *ZiyanAdpt) ListSecurityGroupRule(kt *kit.Kit, opt *securitygrouprule.TCloudListOption) (
	*vpc.SecurityGroupPolicySet, error) {
	...
	if err != nil {
		logs.Errorf("list tcloud ziyan security group rules failed, err: %v, rid: %s", err, kt.Rid)
		return nil, err
	}
