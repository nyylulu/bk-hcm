...
// GetBpaasApplicationDetailRequestParams Predefined struct for user
type GetBpaasApplicationDetailRequestParams struct {
	// 申请单id
	ApplicationId *uint64 `json:"ApplicationId,omitnil" name:"ApplicationId"`
}

// GetBpaasApplicationDetailRequest ...
type GetBpaasApplicationDetailRequest struct {
	*tchttp.BaseRequest
...
// ToJsonString ...
func (r *GetBpaasApplicationDetailRequest) ToJsonString() string {
	b, _ := json.Marshal(r)
	return string(b)
}
...
func (r *GetBpaasApplicationDetailRequest) FromJsonString(s string) error {
	...
	if len(f) > 0 {
		return tcerr.NewTencentCloudSDKError("ClientError.BuildRequestError",
			"GetBpaasApplicationDetailRequest has unknown keys!", "")
	}
	return json.Unmarshal([]byte(s), &r)
}

// GetBpaasApplicationDetailResponseParams Predefined struct for user
type GetBpaasApplicationDetailResponseParams struct {
	// 申请人uin
	...
	// 申请单状态 0 待审批;1 审批通过;2 拒绝;18外部审批中
	// 注意：此字段可能返回 null，表示取不到有效值。
	Status *uint64 `json:"Status,omitnil" name:"Status"`

	// 节点信息
	// 注意：此字段可能返回 null，表示取不到有效值。
	// 返回值类型并不完全符合`[]*StatusNode`类型，暂时改为json.RawMessage
	Nodes *json.RawMessage `json:"Nodes,omitnil" name:"Nodes"`
...
// GetBpaasApplicationDetailResponse ...
type GetBpaasApplicationDetailResponse struct {
	*tchttp.BaseResponse
	Response *GetBpaasApplicationDetailResponseParams `json:"Response"`
}

// ToJsonString ...
func (r *GetBpaasApplicationDetailResponse) ToJsonString() string {
	b, _ := json.Marshal(r)
	return string(b)
}
...
// FromJsonString It is highly **NOT** recommended to use this function
// because it has no param check, nor strict type check
func (r *GetBpaasApplicationDetailResponse) FromJsonString(s string) error {
	return json.Unmarshal([]byte(s), &r)
}
...
const (
	// BPaasApplicationStatusPending 0：待审批
	BPaasApplicationStatusPending uint64 = 0
	// BPaasApplicationStatusApproved 1：审批通过
	BPaasApplicationStatusApproved uint64 = 1
	// BPaasApplicationStatusRejected 2：拒绝
	BPaasApplicationStatusRejected uint64 = 2
	// BPaasApplicationStatusScfFailed 3：scf执行失败
	BPaasApplicationStatusScfFailed uint64 = 3
	// BPaasApplicationStatusScfSucceed 4：scf执行成功
	BPaasApplicationStatusScfSucceed uint64 = 4
	// BPaasApplicationStatusOutsidePending 18: 外部审批中x
	BPaasApplicationStatusOutsidePending uint64 = 18
)
