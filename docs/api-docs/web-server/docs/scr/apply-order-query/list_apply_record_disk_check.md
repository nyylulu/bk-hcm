...
POST /api/v1/woa/task/find/apply/record/disk_check
...
| info    | object array | 资源本地盘压测记录信息列表  |
...
| ip	      | string	  | 资源IP        |
| task_id	  | string	  | 本地盘压测任务ID |
| task_link	  | string	  | 本地盘压测任务详情链接 |
| status	  | int	      | 本地盘压测记录状态码，0: 成功, 1: 执行中, 其他: 失败 |
| message	  | string	  | 本地盘压测记录状态信息 |
| start_at	  | timestamp | 记录开始时间 |
| end_at	  | timestamp | 记录结束时间 |
