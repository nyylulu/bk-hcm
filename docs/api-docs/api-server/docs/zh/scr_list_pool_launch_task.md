...
| 参数名称      | 参数类型       | 必选 | 描述        |
|--------------|--------------|------|------------|
| id           | int array    | 否   | 上架任务ID列表，数量最大20 |
...
| 参数名称     | 参数类型 | 描述                         |
|-------------|--------|------------------------------|
| phase       | string | 上架任务执行阶段。INIT：待执行，RUNNING：执行中，PAUSED：已暂停，SUCCESS：执行成功，FAILED：执行失败 |
| message     | string | 上架任务信息   |
| total_num   | int    | 总上架设备数   |
| success_num | int    | 成功上架设备数 |
| pending_num | int    | 待上架设备数   |
| failed_num  | int    | 上架失败设备数 |
