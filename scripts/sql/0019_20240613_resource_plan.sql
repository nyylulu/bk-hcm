...
(
    ...
    `remark`        varchar(255) not null default '' comment '需求备注',
...
(
    ...
    `itsm_sn`    varchar(64)  not null default '' comment '关联的ITSM单据编码',
    `itsm_url`   varchar(64)  not null default '' comment '关联的ITSM单据链接',
    `crp_sn`     varchar(64)  not null default '' comment '关联的CRP单据编码',
    `crp_url`    varchar(64)  not null default '' comment '关联的CRP单据链接',
    `message`    varchar(255) not null default '' comment '单据失败信息',
    `created_at` timestamp    not null default current_timestamp comment '该记录创建的时间',
    `updated_at` timestamp    not null default current_timestamp on update current_timestamp comment '该记录更新的时间',
