...
func (cli *client) Cert(kt *kit.Kit, params *SyncBaseParams, opt *SyncCertOption) (*SyncResult, error) {
	...
	logs.Infof("[%s] hcservice sync cert listCertFromCloud success, params: %+v, cloud_cert_count: %d, rid: %s",
		enumor.TCloudZiyan, params, len(certFromCloud), kt.Rid)
	...
	logs.Infof("[%s] hcservice sync cert listCertFromDB success, db_cert_count: %d, rid: %s",
		enumor.TCloudZiyan, len(certFromDB), kt.Rid)
	...
	logs.Infof("[%s] hcservice sync cert diff success, addNum: %d, updateNum: %d, delNum: %d, rid: %s",
		enumor.TCloudZiyan, len(addSlice), len(updateMap), len(delCloudIDs), kt.Rid)
...
func (cli *client) deleteCert(kt *kit.Kit, accountID, region string, delCloudIDs []string) error {
	...
	logs.Infof("[%s] sync cert to delete cert success, accountID: %s, count: %d, rid: %s",
...
func (cli *client) createCert(kt *kit.Kit, accountID string, opt *SyncCertOption,
	...
	newIDs, err := cli.dbCli.TCloudZiyan.BatchCreateCert(kt.Ctx, kt.Header(), createReq)
	if err != nil {
	...
	logs.Infof("[%s] sync cert to create cert success, accountID: %s, count: %d, newIDs: %v, opt: %+v, rid: %s",
		enumor.TCloudZiyan, accountID, len(addSlice), newIDs, opt, kt.Rid)
