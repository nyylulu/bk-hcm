...
func (cli *client) SecurityGroup(kt *kit.Kit, params *SyncBaseParams, opt *SyncSGOption) (*SyncResult, error) {
	...
	if err != nil {
		logs.Errorf("[%s] sg sync sgRule failed. err: %v, accountID: %s, region: %s, rid: %s",
			enumor.TCloudZiyan, err, params.AccountID, params.Region, kt.Rid)
		return nil, err
	}
...
func (cli *client) updateSG(kt *kit.Kit, accountID string,
	...
	logs.Infof("[%s] sync sg to update sg success, accountID: %s, count: %d, rid: %s", enumor.TCloudZiyan,
...
func (cli *client) createSG(kt *kit.Kit, accountID string, region string,
	...
	results, err := cli.dbCli.TCloudZiyan.SecurityGroup.BatchCreateSecurityGroup(kt, createReq)
	if err != nil {
		logs.Errorf("[%s] request dataservice to create ziyan security group failed, err: %v, rid: %s",
			enumor.TCloudZiyan, err, kt.Rid)
		return nil, err
	}

	logs.Infof("[%s] sync sg to create sg success, accountID: %s, count: %d, rid: %s", enumor.TCloudZiyan,
...
func (cli *client) deleteSG(kt *kit.Kit, accountID string, region string, delCloudIDs []string) error {
	...
	if len(delSGFromCloud) > 0 {
		logs.Errorf("[%s] validate sg not exist failed, before delete, opt: %v, failed_count: %d, rid: %s",
			enumor.TCloudZiyan, checkParams, len(delSGFromCloud), kt.Rid)
	...
	if err = cli.dbCli.Global.SecurityGroup.BatchDeleteSecurityGroup(kt.Ctx, kt.Header(), deleteReq); err != nil {
		logs.Errorf("[%s] request dataservice to batch delete sg failed, err: %v, rid: %s", enumor.TCloudZiyan,
			err, kt.Rid)
		return err
	}

	logs.Infof("[%s] sync sg to delete sg success, accountID: %s, count: %d, rid: %s", enumor.TCloudZiyan,
...
func (cli *client) listSGFromDB(kt *kit.Kit, params *SyncBaseParams) (
	...
	result, err := cli.dbCli.TCloudZiyan.SecurityGroup.ListSecurityGroupExt(kt, req)
	if err != nil {
		logs.Errorf("[%s] list sg from db failed, err: %v, account: %s, req: %v, rid: %s", enumor.TCloudZiyan,
...
func (cli *client) RemoveSecurityGroupDeleteFromCloudV2(kt *kit.Kit, accountID string, region string,
	...
	for {
		resultFromDB, err := cli.dbCli.TCloudZiyan.SecurityGroup.ListSecurityGroupExt(kt, req)
		if err != nil {
	...
	logs.Infof("[%s] will remove %d deleted security group from cloud, account: %s, region: %s, rid: %s",
		enumor.Ziyan, len(delCloudIDs), accountID, region, kt.Rid)
...
func (cli *client) RemoveSecurityGroupDeleteFromCloud(kt *kit.Kit, accountID string, region string) error {
	...
	for {
		resultFromDB, err := cli.dbCli.TCloudZiyan.SecurityGroup.ListSecurityGroupExt(kt, req)
		if err != nil {
			logs.Errorf("[%s] request dataservice to list sg failed, err: %v, req: %v, rid: %s", enumor.TCloudZiyan,
				err, req, kt.Rid)
			return err
		}
