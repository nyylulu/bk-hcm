...
// SyncSecurityGroup sync tcloud ziyan security group
...
func (hd *sgHandler) Next(kt *kit.Kit) ([]securitygroup.TCloudSG, error) {
	...
	if err != nil {
		logs.Errorf("request adaptor list tcloud ziyan sg failed, err: %v, opt: %v, rid: %s", err, listOpt, kt.Rid)
		return nil, err
	}
...
func (hd *sgHandler) Sync(kt *kit.Kit, instances []securitygroup.TCloudSG) error {
	...
	if _, err := hd.syncCli.SecurityGroup(kt, params, new(ziyan.SyncSGOption)); err != nil {
		logs.Errorf("sync tcloud ziyan sg failed, err: %v, opt: %v, rid: %s", err, params, kt.Rid)
		return err
	}
...
func (hd *sgHandler) RemoveDeletedFromCloud(kt *kit.Kit, allCloudIDMap map[string]struct{}) error {
	...
	if err != nil {
		logs.Errorf("remove ziyan sg delete from cloud failed, err: %v, accountID: %s, region: %s, rid: %s", err,
