...
func (svc *service) SyncLoadBalancer(cts *rest.Contexts) (interface{}, error) {
	...
	return nil, handler.ResourceSyncV2[typeclb.TCloudClb](cts, hd)
}
...
func (hd *lbHandler) Next(kt *kit.Kit) ([]typeclb.TCloudClb, error) {
	...
	mu := &sync.Mutex{}
	results := make([]typeclb.TCloudClb, 0, typecore.TCloudQueryLimit*hd.ListConcurrent())
	for i := uint(0); i < hd.ListConcurrent(); i++ {
...
func (hd *lbHandler) Sync(kt *kit.Kit, instances []typeclb.TCloudClb) error {
	...
	opt := &ziyan.SyncLBOption{
		PrefetchedLB: instances,
	}
	if _, err := hd.syncCli.LoadBalancerWithListener(kt, params, opt); err != nil {
		logs.Errorf("sync ziyan load balancer with rel failed, err: %v, opt: %v, rid: %s", err, params, kt.Rid)
		return err
	}
...
func (hd *lbHandler) RemoveDeletedFromCloud(kt *kit.Kit, allCloudIDMap map[string]struct{}) error {
	...
	if err != nil {
		logs.Errorf("remove ziyan clb delete from cloud failed, err: %v, cloud id: %v,account: %s, region: %s, rid: %s",
