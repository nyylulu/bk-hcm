...
func (hd *lblHandler) RemoveDeleteFromCloud(kt *kit.Kit) error {
	params := &ziyan.ListenerSyncRemovedParams{
		AccountID:          hd.request.AccountID,
		Region:             hd.request.Region,
		CloudIDs:           hd.request.CloudIDs,
		BizID:              hd.lbInfo.BkBizID,
	...
	if err := hd.syncCli.RemoveListenerDeleteFromCloud(kt, params); err != nil {
		logs.Errorf("remove load balancer deleted from cloud failed, err: %v, accountID: %s, region: %s, rid: %s", err,
