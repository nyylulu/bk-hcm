...
func (hd *argsTplAddressHandler) Prepare(cts *rest.Contexts) error {
	...
	if err != nil {
		logs.Errorf("request tcloud ziyan argument template address prepare failed, err: %v, rid: %s", err, cts.Kit.Rid)
		return err
	}
...
func (hd *argsTplAddressHandler) Next(kt *kit.Kit) ([]string, error) {
	...
	if err != nil {
		logs.Errorf("request adaptor list tcloud ziyan argument template address failed, opt: %v, err: %v, rid: %s",
			listOpt, err, kt.Rid)
		return nil, err
	}
...
func (hd *argsTplAddressHandler) Sync(kt *kit.Kit, cloudIDs []string) error {
	...
	syncOpt := &ziyan.SyncArgsTplOption{BkBizID: constant.UnassignedBiz}
	if _, err := hd.syncCli.ArgsTplAddress(kt, params, syncOpt); err != nil {
		logs.Errorf("sync tcloud ziyan argument template address failed, opt: %v, err: %v, rid: %s",
			params, err, kt.Rid)
		return err
	}
...
func (hd *argsTplAddressGroupHandler) Next(kt *kit.Kit) ([]string, error) {
	...
	if err != nil {
		logs.Errorf("request adaptor list tcloud ziyan argument template address group failed, opt: %v, err: %v, rid: %s",
			listOpt, err, kt.Rid)
		return nil, err
	}
...
func (hd *argsTplAddressGroupHandler) Sync(kt *kit.Kit, cloudIDs []string) error {
	...
	syncOpt := &ziyan.SyncArgsTplOption{BkBizID: constant.UnassignedBiz}
	if _, err := hd.syncCli.ArgsTplAddressGroup(kt, params, syncOpt); err != nil {
		logs.Errorf("sync tcloud ziyan argument template address group failed, opt: %v, err: %v, rid: %s",
			params, err, kt.Rid)
		return err
	}
...
func (hd *argsTplServiceHandler) Next(kt *kit.Kit) ([]string, error) {
	...
	if err != nil {
		logs.Errorf("request adaptor list tcloud ziyan argument template service failed, opt: %v, err: %v, rid: %s",
			listOpt, err, kt.Rid)
		return nil, err
	}
...
func (hd *argsTplServiceHandler) Sync(kt *kit.Kit, cloudIDs []string) error {
	...
	syncOpt := &ziyan.SyncArgsTplOption{BkBizID: constant.UnassignedBiz}
	if _, err := hd.syncCli.ArgsTplService(kt, params, syncOpt); err != nil {
		logs.Errorf("sync tcloud ziyan argument template service failed, opt: %v, err: %v, rid: %s",
			params, err, kt.Rid)
		return err
	}
...
func (hd *argsTplServiceGroupHandler) Next(kt *kit.Kit) ([]string, error) {
	...
	if err != nil {
		logs.Errorf("request adaptor list tcloud ziyan argument template service group failed, opt: %v, err: %v, rid: %s",
			listOpt, err, kt.Rid)
		return nil, err
	}
...
func (hd *argsTplServiceGroupHandler) Sync(kt *kit.Kit, cloudIDs []string) error {
	...
	syncOpt := &ziyan.SyncArgsTplOption{BkBizID: constant.UnassignedBiz}
	if _, err := hd.syncCli.ArgsTplServiceGroup(kt, params, syncOpt); err != nil {
		logs.Errorf("sync tcloud ziyan argument template service group failed, opt: %v, err: %v, rid: %s",
			params, err, kt.Rid)
		return err
	}
