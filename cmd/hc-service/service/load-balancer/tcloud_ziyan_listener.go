...
// QueryZiyanListenerTargetsByCloudIDs 直接从云上查询监听器RS列表
func (svc *clbSvc) QueryZiyanListenerTargetsByCloudIDs(cts *rest.Contexts) (any, error) {
...
// CreateTCloudZiyanListener 仅创建监听器自身
func (svc *clbSvc) CreateTCloudZiyanListener(cts *rest.Contexts) (interface{}, error) {
	...
	// 插入新的监听器、规则信息到DB
	id, err := svc.createZiyanListenerDB(cts.Kit, req, lbInfo, cloudLblID)
	if err != nil {
...
func (svc *clbSvc) createZiyanListenerDB(kt *kit.Kit, req *protolb.TCloudListenerCreateReq,
	...
	if req.Protocol.IsLayer7Protocol() {
		...
		created, err := svc.dataCli.TCloudZiyan.LoadBalancer.BatchCreateListener(kt, lblCreateReq)
		if err != nil {
