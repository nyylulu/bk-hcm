...
// TCloudZiyanCreateUrlRule 创建url规则
func (svc *clbSvc) TCloudZiyanCreateUrlRule(cts *rest.Contexts) (any, error) {
	...
	tcloudAdpt, err := svc.ad.TCloudZiyan(cts.Kit, listener.AccountID)
	if err != nil {
		return nil, err
	}
	...
	resp, err := svc.dataCli.TCloudZiyan.LoadBalancer.BatchCreateTCloudUrlRule(cts.Kit, createReq)
	if err != nil {
		return nil, err
	}
...
// TCloudZiyanUpdateUrlRule 修改监听器规则
func (svc *clbSvc) TCloudZiyanUpdateUrlRule(cts *rest.Contexts) (any, error) {
	lblID := cts.PathParameter("lbl_id").String()
	if len(lblID) == 0 {
		return nil, errf.New(errf.InvalidParameter, "listener id is required")
	}
	...
	lb, rules, err := svc.getL7RulesWithLbInZiyan(cts.Kit, lblID, []string{ruleID})
	if err != nil {
		return nil, err
	}

	tcloudAdpt, err := svc.ad.TCloudZiyan(cts.Kit, lb.AccountID)
	if err != nil {
		return nil, err
	}
...
// getL7RuleWithLb 查询同一个监听器下的规则 自研云
func (svc *clbSvc) getL7RulesWithLbInZiyan(kt *kit.Kit, lblID string, ruleIDs []string) (*corelb.BaseLoadBalancer,
	[]corelb.TCloudLbUrlRule, error) {

	// 只能查到7层规则
	ruleResp, err := svc.dataCli.TCloudZiyan.LoadBalancer.ListUrlRule(kt, &core.ListReq{
...
// TCloudZiyanBatchDeleteUrlRule 批量删除规则
func (svc *clbSvc) TCloudZiyanBatchDeleteUrlRule(cts *rest.Contexts) (any, error) {
	lblID := cts.PathParameter("lbl_id").String()
	if len(lblID) == 0 {
		return nil, errf.New(errf.InvalidParameter, "listener id is required")
	}
	...
	// 指定规则id删除
	lb, rules, err := svc.getL7RulesWithLbInZiyan(cts.Kit, lblID, req.RuleIDs)
	if err != nil {
...
// TCloudZiyanBatchDeleteUrlRuleByDomain 按域名批量删除规则
func (svc *clbSvc) TCloudZiyanBatchDeleteUrlRuleByDomain(cts *rest.Contexts) (any, error) {
	lblID := cts.PathParameter("lbl_id").String()
	if len(lblID) == 0 {
		return nil, errf.New(errf.InvalidParameter, "listener id is required")
	}
