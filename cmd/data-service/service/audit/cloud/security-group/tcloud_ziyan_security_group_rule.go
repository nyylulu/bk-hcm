...
func (s *SecurityGroup) tcloudZiyanSGRuleUpdateAuditBuild(kt *kit.Kit, sg tablecloud.SecurityGroupTable,
...
func (s *SecurityGroup) tcloudZiyanSGRuleDeleteAuditBuild(kt *kit.Kit, sg tablecloud.SecurityGroupTable,
...
func (s *SecurityGroup) listTCloudZiyanSGRule(kt *kit.Kit, sgID string, ids []string) (
	map[string]tablecloud.TCloudZiyanSecurityGroupRuleTable, error) {
	...
	list, err := s.dao.TCloudZiyanSGRule().List(kt, opt)
	if err != nil {
		logs.Errorf("list tcloud ziyan security group rule failed, err: %v, ids: %v, rid: %s", err, ids, kt.Rid)
		return nil, err
	}

	result := make(map[string]tablecloud.TCloudZiyanSecurityGroupRuleTable, len(list.Details))
