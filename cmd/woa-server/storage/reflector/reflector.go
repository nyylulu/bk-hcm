...
func (r *Reflector) loopWatch(coll string, w *types.Watcher, cap *Capable) {
	for event := range w.EventChan {
		...
		case types.Invalidate:
			logs.Errorf("watch collection: %s, received a invalidate event, doc: %v.", coll, event.Document)
		default:
			logs.Errorf("watch collection: %s, received a unsupported event type: %s, doc: %v.", coll,
