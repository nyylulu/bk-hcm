...
func (lw *LoopsWatch) WithOne(opts *types.LoopOneOptions) error {
	if err := opts.Validate(); err != nil {
		logs.Errorf("run loop watch, but option is invalid, err: %v", err)
		return err
	}

	startToken, err := opts.TokenHandler.GetStartWatchToken(context.Background())
	if err != nil {
		logs.Errorf("%s job, run loop watch %s, but get start token failed, err: %v", opts.Name,
	...
	if err != nil {
		logs.Errorf("%s job, run loop, but watch failed, err: %v", opts.Name, err)
		cancel()
		return err
	}
...
func (lw *LoopsWatch) WithBatch(opts *types.LoopBatchOptions) error {
	if err := opts.Validate(); err != nil {
		logs.Errorf("run loop watch batch, but option is invalid, err: %v", err)
		return err
	}

	startToken, err := opts.TokenHandler.GetStartWatchToken(context.Background())
	if err != nil {
		logs.Errorf("%s job, run loop watch batch %s, but get start token failed, err: %v", opts.Name,
			opts.WatchOpt.Collection, err)
		return err
	}
...
func (lw *LoopsWatch) watchRetry(cancel context.CancelFunc,
	...
	for {
		...
		case <-opts.StopNotifier:
			cancel()
			logs.Warnf("received stop %s loop watch job notify, stopping now.", opts.Name)
			return
		...
		case <-retrySignal:
			...
			if err != nil {
				logs.Errorf("%s job, run loop watch, but get last event token failed, err: %v", opts.Name, err)
				// notify retry signal, exit loop
				close(retrySignal)
				continue
			}

			logs.Errorf("%s job, the former watch loop: %s failed, start retry again from token: %s.",
			...
			if err != nil {
				...
				// notify retry signal, exit loop
				close(retrySignal)
				logs.Errorf("%s job, run retry loop, but watch failed, err: %v", err)
				continue
			}
			...
			logs.Warnf("%s job, retry loop %s from token: %s success.", opts.Name, opts.WatchOpt.Collection, lastToken)
		}
	}
}
...
func (lw *LoopsWatch) tryLoopWithBatch(ctxWithCancel context.Context,
	...
	for {
		...
		if reWatch {
			...
			ticker.Stop()
			logs.Warnf("%s job, master status has changed, try to re-watch again, collection：%s", opts.Name,
		...
		if !loop {
			logs.V(5).Infof("%s job, loop %s event, but not master, skip.", opts.Name, opts.WatchOpt.Collection)
		...
		for {
			...
			case <-ctxWithCancel.Done():
				...
				logs.Warnf("%s job, received cancel loop watch %s signal, exit loop.", opts.Name,
					opts.WatchOpt.Collection)
				// exist the goroutine
				return
			...
			case one := <-watcher.EventChan:
				...
				if logs.V(4) {
					logs.Infof("%s job, received %s event, detail: %s, op-time: %s, rid: %s", opts.Name,
		...
		logs.Infof("%s job, received %s batch %d events, first op-time: %s rid: %s.", opts.Name,
			opts.WatchOpt.Collection,
		...
		if retry {
			...
			if retryObserver.canStillRetry() {
				logs.Warnf("%s job, received %s %d events in batch, but do batch failed, retry now, rid: %s", opts.Name,
			...
			logs.Warnf("%s job, collection %s batch watch retry exceed max count, skip, rid: %s.", opts.Name,
		...
		if err := opts.TokenHandler.SetLastWatchToken(ctxWithCancel, last.Token.Data); err != nil {
			logs.Errorf("%s job, loop watch %s event, but set last token failed, err: %v, rid: %s, retry later.",
...
func (lw *LoopsWatch) tryLoopWithOne(ctxWithCancel context.Context,
	...
	for one := range watcher.EventChan {
		select {
		case <-ctxWithCancel.Done():
			logs.Warnf("%s job, received cancel loop watch %s signal, exit loop, exit loop", opts.Name,
				opts.WatchOpt.Collection)
			return
		...
		reWatch, loop := observer.canLoop()
		if reWatch {
			logs.Warnf("%s job, master status has changed, try to re-watch %s again", opts.Name,
				opts.WatchOpt.Collection)
			// trigger re-watch action now.
			close(retrySignal)
			// exit the for loop
		...
		if !loop {
			logs.Infof("%s job, received %s %s event, but not master, skip. details: %s, rid: %s",
				opts.Name, opts.WatchOpt.Collection, one.OperationType, one.String(), one.ID())
			continue
		}

		logs.Infof("%s job, received %s event, type: %s, op-time: %s rid: %s", opts.Name, opts.WatchOpt.Collection,
			one.OperationType, one.ClusterTime.String(), one.ID())

		if logs.V(4) {
			logs.Infof("%s job, event details: %s, oid: %s", opts.Name, one.String(), one.ID())
		}

		retry := lw.tryOne(one, opts)
		if retry {
			if retryObserver.canStillRetry() {
				logs.Warnf("%s job, retry watch %s later. rid: %s", opts.Name, opts.WatchOpt.Collection, one.ID())
				// an error occurred, we need to retry it later.
				// tell the schedule to re-watch again.
				close(retrySignal)
				// exist this goroutine.
			...
			logs.Warnf("%s job, retry %s event exceed max count, skip, detail: %s, rid: %s", opts.Name,
		...
		if err := opts.TokenHandler.SetLastWatchToken(ctxWithCancel, one.Token.Data); err != nil {
			logs.Errorf("%s job, loop watch %s event, but set last watched token failed, err: %v, rid: %s, "+
				"retry later.",
...
func (lw *LoopsWatch) tryOne(e *types.Event, opts *types.LoopOneOptions) (retry bool) {
	...
	case types.Insert:
		...
		if retry {
			logs.Warnf("%s job, received %s %s event,  but do add job failed, retry now, rid: %s", opts.Name,
	...
	case types.Update, types.Replace:
		...
		if retry {
			logs.Warnf("%s job, received %s %s event, but do update job failed, retry now, rid: %s", opts.Name,
				opts.WatchOpt.Collection, e.OperationType, e.ID())
	...
	case types.Delete:
		...
		if retry {
			logs.Warnf("%s job, received %s %s event, but do delete job failed, retry now, rid: %s", opts.Name,
				opts.WatchOpt.Collection, e.OperationType, e.ID())
	...
	case types.Invalidate:
		logs.Errorf("%s job, watch %s event, received invalid operation type, doc: %s, rid: %s", opts.Name,
			opts.WatchOpt.Collection, e.DocBytes, e.ID())
		return false

	default:
		logs.Errorf("%s job, watch %s event, received unsupported operation type, doc: %s, rid: %s", opts.Name,
			opts.WatchOpt.Collection, e.DocBytes, e.ID())
		return false
	}
...
func (o *observer) canLoop() (reWatch bool, loop bool) {
	...
	logs.Infof("loop watch, is master status changed from %v to %v.", o.previousStatus, current)
