...
func (e *Event) Watch(ctx context.Context, opts *types.WatchOptions) (*types.Watcher, error) {
	...
	go func() {
		...
		logs.Infof("start watch with pipeline: %+v, options: %+v, stream options: %+v", pipeline, opts, streamOptions)
		...
		if err != nil && isFatalError(err) {
			...
			logs.Errorf("mongodb watch collection: %s got a fatal error, skip resume token and retry, err: %v",
				opts.Collection, err)
			...
			if opts.WatchFatalErrorCallback != nil {
				...
				if err != nil {
					logs.Errorf("do watch fatal error callback for coll %s failed, err: %v", opts.Collection, err)
				}
			}

			logs.Infof("start watch with pipeline: %+v, options: %+v, stream options: %+v", pipeline, opts,
				streamOptions)
			if opts.Collection != "" {
				stream, err = e.client.
					Database(e.database).
		...
		if err != nil {
			...
			logs.Fatalf("mongodb watch failed with conf: %+v, err: %v", *opts, err)
		}
...
func (e *Event) loopWatch(ctx context.Context,
	...
	for {
		...
		select {
		case <-ctx.Done():
			logs.Warnf("received stopped loop watch signal, stop watch db: %s, collection: %s, err: %v", e.database,
		...
		if retry {
			...
			if streamOptions.StartAtOperationTime != nil && streamOptions.StartAfter != nil {
				logs.Infof("resume token and time is both set, discard the resume time, option: %+v", streamOptions)
				streamOptions.StartAtOperationTime = nil
			}

			logs.Infof("retry watch with pipeline: %+v, opts: %+v, stream opts: %+v", pipeline, opts, streamOptions)
			...
			if err != nil {
				if isFatalError(err) {
					...
					if opts.WatchFatalErrorCallback != nil {
						err := opts.WatchFatalErrorCallback(types.TimeStamp{Sec: startAtTime})
						if err != nil {
							logs.Errorf("do watch fatal error callback for coll %s failed, err: %v", opts.Collection,
								err)
						}
					}
				}

				logs.Errorf("mongodb watch %s failed with opts: %+v, pipeline: %+v, streamOpts: %+v, err: %v",
		...
		for stream.Next(ctx) {
			...
			select {
			case <-ctx.Done():
				logs.Warnf("received stopped loop watch signal, stop loop next, watch db: %s, collection: %s, err: %v",
			...
			if err := stream.Decode(newStruct.Addr().Interface()); err != nil {
				logs.Errorf("watch collection %s, but decode to event struct: %v failed, err: %v",
			...
			if base.OperationType == types.Invalidate {
				logs.ErrorJson("mongodb watch received a invalid event, will retry watch again, options: %s", *opts)
		...
		if err := stream.Err(); err != nil {
			logs.Errorf("mongodb watch encountered a error, conf: %+v, err: %s", *opts, err)
			stream.Close(ctx)
			retry = true
			continue
		}
...
func (e *Event) setCleaner(ctx context.Context, eventChan chan *types.Event, coll string) {
	go func() {
		select {
		case <-ctx.Done():
			logs.Warnf("received stopped loop watch collection: %s signal, close cursor now, err: %v",
				coll, ctx.Err())
			...
			case <-eventChan:
				logs.Warnf("received stopped loop watch collection: %s signal, consumed a event", coll)
