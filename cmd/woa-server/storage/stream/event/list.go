...
func (e *Event) lister(ctx context.Context, withRetry bool, cnt int64, opts *types.ListOptions, ch chan *types.Event) {
	...
	retry:
		...
		if err != nil {
			logs.Errorf("list watch operation, but list db: %s, collection: %s failed, will *retry later*, err: %v",
		...
		for cursor.Next(ctx) {
			select {
			case <-ctx.Done():
				logs.Errorf("received stopped lister signal, stop list db: %s, collection: %s, err: %v", e.database,
			...
			if err != nil {
				logs.Errorf("list watch operation, but list db: %s, collection: %s with cursor failed, will *retry later*, err: %v",
					e.database, opts.Collection, err)

				cursor.Close(ctx)
				if !withRetry {
					logs.Warnf("list watch operation, but list db: %s, collection: %s with cursor failed, will exit list immediately.",
