...
// Package apply order informer handler
package apply
...
// applyHandler apply order informer handler
type applyHandler struct {
	key     Key
	watchDB dal.DB
}

// newApplyTokenHandler creates an apply order informer token handler
func newApplyTokenHandler(key Key, watchDB dal.DB) *applyHandler {
	return &applyHandler{
		key:     key,
		watchDB: watchDB,
	}
...
func (h *applyHandler) SetLastWatchToken(ctx context.Context, token string) error {
	return nil
}

// setLastWatchToken set last watch token used after events are successfully inserted
func (h *applyHandler) setLastWatchToken(ctx context.Context, data map[string]interface{}) error {
	...
	if err := h.watchDB.Table(common.BKTableNameWatchToken).Update(ctx, filter, tokenInfo); err != nil {
		logs.Errorf("set apply order %s last watch token failed, err: %v, data: %+v", h.key.Collection(),
			err, tokenInfo)
		return err
	}
	return nil
...
func (h *applyHandler) GetStartWatchToken(ctx context.Context) (token string, err error) {
	filter := map[string]interface{}{
		"_id": "cr_applyOrder",
	}
	...
	if err := h.watchDB.Table(common.BKTableNameWatchToken).Find(filter).Fields(h.key.Collection()).
		...
		if !h.watchDB.IsNotFoundError(err) {
			logs.ErrorJson("get apply order start watch token, will get the last event's time and start watch, "+
				"err: %s, filter: %s", err, filter)
		}

		tailNode := new(watch.ChainNode)
		if err := h.watchDB.Table("cr_applyOrder").Find(nil).Fields(common.BKTokenField).
			Sort(common.BKFieldID+":-1").One(context.Background(), tailNode); err != nil {

			if !h.watchDB.IsNotFoundError(err) {
				logs.Errorf("get apply order last watch token from mongo failed, err: %v", err)
				return "", err
			}
