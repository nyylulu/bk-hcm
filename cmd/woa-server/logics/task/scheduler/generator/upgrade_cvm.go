...
func (g *Generator) UpgradeCVMSync(kt *kit.Kit, order *types.ApplyOrder) (orderID string, err error) {
	...
	if err := record.StartStep(order.SubOrderId, types.StepNameGenerate); err != nil {
		logs.Errorf("failed to start generate step, order id: %s, err: %v", order.SubOrderId, err)
		return "", err
	}
	...
	if err != nil {
		logs.Errorf("failed to upgrade cvm, suborder id: %s", order.SubOrderId)
		return "", err
	}
	return orderID, nil
}
...
func (g *Generator) launchUpgradeCVM(kt *kit.Kit, reqParam *cvmapi.UpgradeParam, order *types.ApplyOrder) (
	...
	if err != nil {
		logs.Warnf("failed to marshal upgrade cvm req, req: %v, err: %v", req, err)
	}
	...
	if err != nil {
		logs.Warnf("failed to marshal cvm upgrade order create resp, resp: %v, err: %v", resp, err)
	}
