...
func SetHeaderFromApiGW(header http.Header, userName, appCode string) {
	header.Set(common.BKHTTPLanguage, header.Get(common.BKHTTPAPIGWLanguage))
	header.Set(common.BKHTTPOwnerID, header.Get(common.BKHTTPAPIGWOwnerID))
	header.Set(common.BKHTTPHeaderUser, userName)
	header.Set(common.BKHTTPRequestAppCode, appCode)
}
