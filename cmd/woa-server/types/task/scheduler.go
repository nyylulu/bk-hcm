...
func (s *ResourceSpec) ValidateDisk() (string, error) {
	...
	if len(s.SystemDisk.DiskType) > 0 {
		if err := s.SystemDisk.Validate(); err != nil {
			return "system_disk.disk_type", err
		}
		if s.SystemDisk.DiskSize < 50 || s.SystemDisk.DiskSize > 1000 {
			return "system_disk.disk_size", fmt.Errorf("system_disk_size invalid value, must be in range [50, 1000]")
		}
		// 系统盘大小必须是50的倍数
		if s.SystemDisk.DiskSize%50 != 0 {
			return "system_disk.disk_size", fmt.Errorf("system_disk_size must be a multiple of 50")
		}
	}
	...
	if len(s.DataDisk) > 0 {
		...
		for _, dd := range s.DataDisk {
			...
			if dd.DiskSize < 10 || dd.DiskSize > 32000 {
				return "data_disk.disk_size", fmt.Errorf("data_disk_size invalid value, must be in range [10, 32000]")
			}
			// 数据盘大小必须是10的倍数
			if dd.DiskSize%10 != 0 {
				return "data_disk.disk_size", fmt.Errorf("data_disk_size must be a multiple of 10")
			}
			dataDiskTotalNum += dd.DiskNum
		}
		// 数据盘总数量不能超过20块
		if dataDiskTotalNum < 0 || dataDiskTotalNum > 20 {
			return "data_disk.disk_size", fmt.Errorf("data_disk_total_num invalid value, must be in range [0, 20]")
		}
	}
