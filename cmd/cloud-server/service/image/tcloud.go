...
// QueryImage ...
func (svc *imageSvc) QueryImage(cts *rest.Contexts) (interface{}, error) {
	...
	switch vendor {
	case enumor.TCloud:
		return svc.TCloudQueryImage(cts)
	case enumor.TCloudZiyan:
		return svc.TCloudZiyanQueryImage(cts)
	default:
		return nil, errf.NewFromErr(errf.InvalidParameter, fmt.Errorf("query image no support vendor: %s", vendor))
	}
}
...
// QueryBizImage ...
func (svc *imageSvc) QueryBizImage(cts *rest.Contexts) (interface{}, error) {
	vendor := enumor.Vendor(cts.PathParameter("vendor").String())
	if len(vendor) == 0 {
		return nil, errf.New(errf.InvalidParameter, "vendor is required")
	}
	...
	switch vendor {
	case enumor.TCloud:
		return svc.TCLoudBizQueryImage(cts)
	case enumor.TCloudZiyan:
		return svc.TCLoudZiyanBizQueryImage(cts)
	default:
		return nil, errf.NewFromErr(errf.InvalidParameter, fmt.Errorf("query biz image no support vendor: %s", vendor))
	}
}
