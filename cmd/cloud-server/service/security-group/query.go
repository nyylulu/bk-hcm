...
func (svc *securityGroupSvc) getSecurityGroup(cts *rest.Contexts, validHandler handler.ValidWithAuthHandler) (
	...
	case enumor.TCloudZiyan:
		return svc.getTCloudZiyanSecurityGroup(cts.Kit, id, cvmCount)
	default:
...
func (svc *securityGroupSvc) getTCloudZiyanSecurityGroup(kt *kit.Kit, id string, cvmCount uint64) (
	*proto.SecurityGroup[corecloud.TCloudSecurityGroupExtension], error) {

	sg, err := svc.client.DataService().TCloudZiyan.SecurityGroup.GetSecurityGroup(kt, id)
	if err != nil {
		return nil, err
	}
