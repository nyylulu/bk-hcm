...
func (svc *lbSvc) buildCreateTCloudTarget(kt *kit.Kit, body json.RawMessage, accountID string,
	...
	for lbID, lbTgIDs := range lbTgsMap {
		...
		result, err := svc.buildAddTCloudTarget(kt, addTargetJSON, accountID, vendor)
		if err != nil {
			return nil, err
		}
	...
	for _, tmpRsIP := range req.RsIP {
		...
		if len(cvmList.Details) != 1 {
			...
			return nil, errf.NewFromErr(errf.RecordNotFound,
				fmt.Errorf("one rs_ip: %s should correspond to one CVM, but %d were found", tmpRsIP,
					len(cvmList.Details)))
		}
...
func (svc *lbSvc) parseSOpsTargetParamsForRsOnline(kt *kit.Kit, accountID string, vendor enumor.Vendor,
	...
	for index, item := range tgQueryList {
		...
		protoDomainUrlTgIDs, err := svc.parseSOpsProtocolAndDomainAndUrlForTgIDs(kt, accountID, vendor, item.Protocol,
