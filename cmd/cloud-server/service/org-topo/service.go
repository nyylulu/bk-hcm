...
func (ots *orgTopoSvc) SyncOrgTopos(cts *rest.Contexts) (any, error) {
	if err := ots.authorizer.AuthorizeWithPerm(cts.Kit, meta.ResourceAttribute{Basic: &meta.Basic{
		Type: meta.ZiyanResDissolveManage, Action: meta.Find}}); err != nil {
		logs.Errorf("no permission to sync usermgr org topo resource, err: %v, rid: %s", err, cts.Kit.Rid)
		return nil, err
	}
