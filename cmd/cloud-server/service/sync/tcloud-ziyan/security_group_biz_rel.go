...
func SyncSGUsageBizRel(kt *kit.Kit, cliSet *client.ClientSet, accountID string, regions []string,
	...
	start := time.Now()
	logs.V(3).Infof("tcloud-ziyan account[%s] sync sg usage biz rel start, time: %v, rid: %s",
	...
	defer func() {
		logs.V(3).Infof("tcloud-ziyan account[%s] sync sg usage biz end, cost: %v, rid: %s",
	...
	for _, region := range regions {
		...
		err := cliSet.HCService().TCloudZiyan.SecurityGroup.SyncSecurityGroupUsageBizRel(kt, req)
		if err != nil {
