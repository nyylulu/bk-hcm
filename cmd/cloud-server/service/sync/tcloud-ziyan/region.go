...
func SyncRegion(kt *kit.Kit, hcCli *hcservice.Client, accountID string) error {
	...
	start := time.Now()
	logs.V(3).Infof("tcloud ziyan account[%s] sync region start, time: %v, rid: %s", accountID, start, kt.Rid)

	defer func() {
		logs.V(3).Infof("tcloud ziyan account[%s] sync region end, cost: %v, rid: %s",
	...
	if err := hcCli.TCloudZiyan.Region.Sync(kt, req); err != nil {
		logs.Errorf("sync tcloud ziyan region failed, err: %v, req: %v, rid: %s", err, req, kt.Rid)
		return err
	}
...
func ListRegion(kt *kit.Kit, dataCli *dataservice.Client) ([]string, error) {
	...
	if err != nil {
		logs.Errorf("list tcloud ziyan region failed, err: %v, rid: %s", err, kt.Rid)
		return nil, err
	}

	if len(result.Details) == 0 {
		return nil, errors.New("tcloud ziyan region is empty")
	}
