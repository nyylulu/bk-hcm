...
func SyncLoadBalancer(kt *kit.Kit, cliSet *client.ClientSet, accountID string, regions []string,
	...
	start := time.Now()
	logs.V(3).Infof("tcloud ziyan account[%s] sync load balancer start, time: %v, rid: %s", accountID, start, kt.Rid)
	...
	defer func() {
		logs.V(3).Infof("tcloud ziyan account[%s] sync load balancer end, cost: %v, rid: %s",
	...
	for _, region := range regions {
		...
		if err := cliSet.HCService().TCloudZiyan.Clb.SyncLoadBalancer(kt, req); err != nil {
			logs.Errorf("sync tcloud ziyan load balancer failed, err: %v, req: %v, rid: %s", err, req, kt.Rid)
			return err
		}
	}
