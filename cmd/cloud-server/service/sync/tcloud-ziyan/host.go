...
func SyncHost(kt *kit.Kit, cliSet *client.ClientSet, accountID string, sd *detail.SyncDetail) error {
	...
	start := time.Now()
	logs.V(3).Infof("tcloud ziyan account[%s] sync host start, time: %v, rid: %s", accountID, start, kt.Rid)
	...
	defer func() {
		logs.V(3).Infof("tcloud ziyan account[%s] sync host end, cost: %v, rid: %s", accountID, time.Since(start),
			kt.Rid)
	}()

	bizIDs, err := listIEGBizIDs(kt)
	if err != nil {
	...
	for _, bizID := range bizIDs {
		...
		req := &sync.TCloudZiyanSyncHostReq{
			AccountID: accountID,
			BizID:     bizID,
		}
		if err = cliSet.HCService().TCloudZiyan.Cvm.SyncHostWithRelResource(kt.Ctx, kt.Header(), req); err != nil {
			logs.Errorf("sync tcloud ziyan host failed, err: %v, req: %v, rid: %s", err, req, kt.Rid)
			return err
		}
...
func listIEGBizIDs(kt *kit.Kit) ([]int64, error) {
	iegRule := &cmdb.AtomRule{
		Field:    "bk_operate_dept_id",
		Operator: "equal",
		Value:    3, // 这里只需要查询ieg的数据
	}
