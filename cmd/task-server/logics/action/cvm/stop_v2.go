...
// Name ...
func (c StopActionV2) Name() enumor.ActionName {
	return enumor.ActionStopCvmV2
}

// Run ...
func (c StopActionV2) Run(kt run.ExecuteKit, params interface{}) (result interface{}, err error) {
...
func (c StopActionV2) stopCvm(kt *kit.Kit, opt *CvmOperationOptionV2) error {

	switch opt.Vendor {
	case enumor.TCloud:
		err := c.stopTCloudCvm(kt, opt)
		if err != nil {
			logs.Errorf("fail to start tcloud cvm, err: %v, req: %+v, rid: %s", err, opt, kt.Rid)
			return err
		}
	case enumor.TCloudZiyan:
		err := c.stopTCloudZiyanCvm(kt, opt)
		if err != nil {
...
func (c StopActionV2) stopTCloudCvm(kt *kit.Kit, opt *CvmOperationOptionV2) error {
	...
	executeErr := actcli.GetHCService().TCloud.Cvm.BatchStopCvm(kt, req)
	if executeErr != nil {
