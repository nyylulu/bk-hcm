...
import { ModelPropertyForm } from '@/model/typings';
...
import type { ISearchProps, ISearchCondition } from '../typings';
...
const fieldIds = ['bk_biz_ids', 'date'];
const fields = fieldIds.map((id) => properties.find((prop) => prop.id === id));

const formValues = ref<ISearchCondition>({});
let conditionInitValues: ISearchCondition;

const getSearchCompProps = (field: ModelPropertyForm) => {
  if (field.id === 'date') {
    return {
      type: 'daterange',
      format: 'yyyy-MM-dd',
      disabledDate: (date: Date) => dayjs(date).isAfter(dayjs()),
      clearable: false,
    };
  }
  return {
