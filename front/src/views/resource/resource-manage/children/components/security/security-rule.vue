...
const filter = ref({
  op: 'and',
  rules: [{ field: 'type', op: 'eq', value: 'ingress' }],
});
          ...
          withDirectives(
            h(
              Button,
              {
                ...
                theme: 'primary',
                class: {
                  'hcm-no-permision-text-btn': !authVerifyData.value?.permissionAction?.[actionName.value],
                },
                disabled:
                  route.query.vendor === 'huawei' ||
  ...
  <div>
    <section class="rule-main">
      ...
      <div @click="showAuthDialog(actionName)">
        <bk-button
          v-if="!checkVendorInResource(route.query.vendor as string)"
